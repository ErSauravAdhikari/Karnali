...
@auth_api.post('/', tags=_TGS, response={200: UserOut, 403: None}, auth=None)
...
@auth_api.delete('/', tags=_TGS, response={204: None}, auth=django_auth)
def logout(request):
    django_logout(request)
    return 204, None


@auth_api.get('/me', tags=_TGS, response=UserOut, auth=django_auth)
def me(request):
    return request.user


@auth_api.post('/request_password_reset',
               tags=_TGS,
               response={204: None},
               auth=None)
